...
class ASTNodeEncoder(torch.nn.Module):
    ...
    '''
    def __init__(self, emb_dim, num_nodetypes, num_nodeattributes, max_depth, mask=False):
    ...
    def forward(self, x, depth):
        ...
        if depth.max() > self.max_depth:
            new_depth = torch.where(depth > self.max_depth, self.max_depth * torch.ones(depth.shape, dtype=int).to(depth.device), depth)
        else:
            new_depth = depth

        return self.type_encoder(x[:,0]) + self.attribute_encoder(x[:,1]) + self.depth_encoder(new_depth)
